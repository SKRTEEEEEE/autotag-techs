  ...
  async detectAndTag(repoPath: string, includeFull: boolean): Promise<void> {
    ...
    for (const [i, tech] of allTechs.entries()) {
      ...
      try {
        ...
        if (exactMatch) {
          this.logger.info(
            `Found exact API match for ${tech}: ${exactMatch.title} (badge: ${exactMatch.nameBadge ?? exactMatch.slug})`,
          );
          matchedTechs.push(normalizedBadge);
        } else {
          if (results.length > 0) {
            const resultIdentifiers = results
              ...
              .join(", ");
            this.logger.info(
              `Found ${results.length} partial match(es) for ${tech} but no exact match: [${resultIdentifiers}]`,
            );
          } else {
